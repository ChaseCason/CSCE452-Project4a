...
class Navigation(Node):
    ...
    def laser_callback(self, msg):
        #Get min/max angle, ranges, and the increment between angles from the laser scan
        ...
        #Initialize with robot going straight
        goStraight = True
        turnLeft = False
        turnRight = False
        ...
        for i in range(len(ranges)):
            ...
            # Find the x/y distance to the obstacle from center of the robot
            ydist_to_obstacle = r * math.cos(abs(theta)) + radius * 0.5
            xdist_to_obstacle = r * math.sin(abs(theta))

            #Determine if any of the points on the circle will collide with an obstacle in the next second
        ...
        if goStraight:
            ...
            #Determine where to turn. If both left and right then go to the side with more
